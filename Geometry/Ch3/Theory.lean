...
public class BetweenGeometry extends IncidenceGeometry where
  ...
  -- A * B * D, B * C * D, and B * D * E". Here I introduce the line BD explicitly with it's incidence conditions
  -- then posit the proposed points with _their_ incidence conditions, and then finally state the between
  -- condition.
  -- I called this 'density' because, used recursively, it posits something like the density of rationals --
  -- for any two distinct points on a line, there is always a point between them.
  (ba_2_density :
    ∀ B D : Point, B ≠ D ->
    ∃ BD : Line, (B on BD) ∧ (D on BD) ->
    ∃ A C E : Point, (A on BD) ∧ (C on BD) ∧ (E on BD) ->
  ...
  (ba_3_lines_arent_circles :
    ∀ A B C : Point, distinct [A, B, C] ->
    let ABC := Between A B C; let BAC := Between B A C; let ACB := Between A C B;
    (ABC ∧ ¬BAC ∧ ¬ACB) ∨ (¬ABC ∧ ¬BAC ∧ ACB) ∨ (¬ABC ∧ BAC ∧ ¬ACB))

variable [BetweenGeometry]

notation:80 A "*" B "*" C => BetweenGeometry.Between A B C
